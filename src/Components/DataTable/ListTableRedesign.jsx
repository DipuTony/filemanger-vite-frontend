...
function ListTableRedesign(props) {
        ...
        <>
            ...
            <div className=" py-2 overflow-x-auto bg-white">
                <div className="inline-block min-w-full rounded-lg overflow-hidden bg-white">
                    <table {...getTableBodyProps} className="min-w-full ">
                                    ...
                                    <tr {...headerGroup.getHeaderGroupProps()}>
                                        {
                                            headerGroup.headers.map((column) => (
                                                <th {...column.getHeaderProps(column.getSortByToggleProps())} className="px-2 py-3 border-b-2 font-medium border-gray-200 text-gray-600  text-left text-md capitalize">{column.render('Header')}
                        ...
                        <tbody {...getTableBodyProps()} className="text-sm border-b">
                            {/* {rows.map((row) => { */} {/**since used pagination */}
                            {page?.map((row) => {
                                prepareRow(row)
                                return (
                                    <tr {...row.getRowProps()} className="bg-white hover:bg-blue-100 rounded-lg">
                                        {row?.cells?.map((cell ,index) => {
                                            return <td {...cell.getCellProps()} className={`${index == 0 && `rounded-l-[12px] `} ${index == (row?.cells?.length - 1) && `rounded-r-[12px] `} px-2 py-3 text-sm text-left text-gray-600`}>{cell.render('Cell')}</td>
                                        })}
                                    </tr>
                                )
                            })}
...
export default ListTableRedesign
/**
 * Exported to :
